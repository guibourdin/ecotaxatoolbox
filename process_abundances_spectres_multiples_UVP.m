...
% adapted for multiple depth bins by Guillaume Bourdin january 2020
...
zoo_groups = strrep(zoo_groups,'-','_');
...
for dep = 1:size(subset,1)
    for meth = 1:3
        ...
        mj = subset.major{dep}; % (mm)
        mn = subset.minor{dep}; % (mm)               eval(['mn(:,1) = base_zooscan(base_spectres(t).',char(fraction_names(nbr)),'.base_zooscan_index).ids.Minor;']);% (mm�)
        aareaexc = subset.area_exc{dep}; % (mm2)               eval(['aareaexc(:,1) = base_zooscan(base_spectres(t).',char(fraction_names(nbr)),'.base_zooscan_index).ids.Area_exc;']);   % (mm�)
        aarea = subset.area{dep}; % (mm2)               eval(['aarea(:,1) = base_zooscan(base_spectres(t).',char(fraction_names(nbr)),'.base_zooscan_index).ids.Area;']);% (mm�)
%         fferet = subset.perimferet{dep}; % (mm)               eval(['fferet(:,1)
        %% calculating volumes
        if meth == 3
...
%             ar=pi.*(mj./2).*(mn./2);
        ...
        pred = subset.object_annotation_hierarchy{dep};
        id = NaN(size(mj,1),m);
        for j=1:m % m = # de gpes de zoo
            id(:,j) = strcmp(zoo_groups{j,1},pred); % id matrix for initially identified groups
        end
        id = logical(id); % matrice des indices d'identification de chaque objets l = size(pred,1) et c = # de gpes: on a un seul 1 par ligne!!!

        conver = subset.conver{dep}; %per cubic meter
        base_spectres(i).tot.conver{dep}=conver;
% %         eval(['base_spectres(i).tot.conver{dep}=conver;'])
        base_spectres(i).tot.Ab{dep} = NaN(m,1);
        base_spectres(i).tot.Abtot{dep} = NaN(m,1);
        base_spectres(i).tot.Bv{dep} = NaN(m,1);
        base_spectres(i).tot.Bvtot{dep} = NaN(m,1);
        base_spectres(i).tot.Yab{dep} = NaN(size(X,1), m);
        eval(['base_spectres(i).tot.Ybv_',char(method_list(meth)),'{dep} = NaN(size(X,1), m);']);
...
% %             Ab(j) = sum(ID(:,j).*conver); % abundance per fraction rapport�e au volume
            base_spectres(i).tot.Ab{dep}(j,1) = sum(id(:,j).*conver);
...
% %             Abtot(j) = sum(ID(:,j));    % abondance totale sur toute la profondeur echantillonn�e (= # d'organismes EN TOUT, pas rapport� au volume filtr�)
            base_spectres(i).tot.Abtot{dep}(j,1) = sum(id(:,j));
...
% %                 Bv(j) = sum(ID(:,j).*vol.*conver) ;% biovolume per fraction rapport�e au volume
                base_spectres(i).tot.Bv{dep}(j,1) = sum(id(:,j).*vol.*conver);
...
% %                 Bvtot(j) = sum(ID(:,j).*vol);    % biovolume total sur toute la profondeur echantillonn�e (= # d'organismes EN TOUT, pas rapport� au volume filtr�)
                base_spectres(i).tot.Bvtot{dep}(j,1) = sum(id(:,j).*vol);
...
%             if ~isempty(aaa)
            if any(id(:,j))
                % ---------- Abundances ---------------------
                eval(['[X,Yab_zoo_groups_',num2str(j),',Z,X1]=f_ZOO_spectrumUVP(sss(id(:,j),1),vol(id(:,j),1),smin,smax,k,1,conver(id(:,j)));']);% clear aaa
                eval(['Yab_zoo_groups_',num2str(j),'=Yab_zoo_groups_',num2str(j),';']) %'Ygroup'= size distribution for a group for a fraction

                % ---------- Biovolume ---------------------
                eval(['[X,Ybv_zoo_groups_',num2str(j),',Z,X1]=f_ZOO_spectrumUVP(sss(id(:,j),1),vol(id(:,j),1),smin,smax,k,2,conver(id(:,j)));']);% clear aaa
            ...
            % ----------- On charge les donnees pour les fractions dans la base ---------
            eval(['base_spectres(i).tot.Yab{dep}(:,j) = Yab_zoo_groups_',num2str(j),';']);
            eval(['base_spectres(i).tot.Ybv_',char(method_list(meth)),'{dep}(:,j) = Ybv_zoo_groups_',num2str(j),';']);

        end
    end
    base_spectres(i).tot.X{dep} = X;
    % % eval(['base_spectres(i).tot.X = X;']);
    base_spectres(i).tot.X1{dep} = X1;
    % % eval(['base_spectres(i).tot.X1 = X1;']);
    base_spectres(i).tot.Zoo_groups{dep} = zoo_groups;
